    ...
    <div className='w-full bg-slate-50 h-screen flex items-center justify-center'>
        <div className='w-[35%] p-20 mx-auto'>
            ...
            <div className='felx-col items-center justify-center'>
                ...
                <button onClick={handleLogin} className={`${isLoading ? 'bg-blue-100 cursor-not-allowed' : 'bg-blue-500 cursor-pointer hover:bg-blue-600'} p-3 w-full shadow-lg rounded-lg  duration-500 text-white mt-10 flex items-center justify-center`} disabled={isLoading}>{isLoading ? <Loader2 className='text-center animate-spin'/> : "Login"}</button>
                <h1 className='text-left text-md flex items-center gap-2 mt-3'>Don't have an account? <p onClick={() => navigate("/register")} className='text-blue-500 hover:underline cursor-pointer hover:text-blue-600'>SignUp</p></h1>
            </div>
        </div>
